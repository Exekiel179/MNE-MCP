...
- **32 个结构化工具** + 一个 **`mne_run_code` 万能工具**：常见流程有专门的工具，做不到的（源定位、连接性、解码、统计、BIDS…）用 `mne_run_code` 在同一个会话里写代码完成。
...
源定位（dSPM/eLORETA）、连接性（mne-connectivity）、解码/MVPA（mne.decoding）、置换检验统计（mne.stats）、BIDS（mne-bids）、HTML 报告（mne.Report）、条件对比、特殊格式读取——都能用 **`mne_run_code`** 在同一个活动会话里完成。会话里已预绑定 `mne`、`np`、`pd`、`plt` 以及你加载的所有对象（`raw`、`epochs`、`evoked`、`ica` …）。
