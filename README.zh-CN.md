...
**没装 `uv`？用纯 Python 引导 `pipx`，再一步装好并注册：**

```bash
python -m pip install --user pipx
python -m pipx ensurepath          # 重开终端，让 pipx 进入 PATH
pipx install mne-mcp               # 轻装；后端按需安装
mne-mcp setup                      # 注册到客户端 + 安装技能
mne-mcp install-backend            # 装 MNE + ICA（或让 mne_install_backend 工具自动装）
```
